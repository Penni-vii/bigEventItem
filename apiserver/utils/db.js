...
module.exports = {
  ...
  getCategory() {
    try {
      let category = JSON.parse(fs.readFileSync(path.join(basePath, 'category.json'), 'utf-8'));
      return category;
  ...
  addComments({ article_id, name, content }) {
    ...
    let id = comments.length ? comments[comments.length - 1].id + 1 : 1;
  ...
  getComments({ article_id = -1, state = 'all' } = {}) {
    try {
      ...
      if (article_id > 0) {
        return comments.filter(item => item.article_id === article_id);
      }
      return comments;
    } catch (error) {
      // const comments = [];
  ...
  passComments(id, isPass) {
    let comments = this.getComments();
    let c = comments.find(item => item.id === id);
    if (!c) {
      return false;
    }
